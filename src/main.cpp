...
void make_robot(std::unordered_map<RobotParts, Part> &container,
                ShaderManager &AnimationEng) {
  Part part(Paths::resources_base_obj.string());
  container.emplace(RobotParts::base, part);
  AnimationEng.addShader(RobotParts::base, Shader(Paths::shaders_vs.c_str(),
                                                  Paths::shaders_fs.c_str()), {1, 0, 0});
  container.emplace(RobotParts::upper_base,
                    Part(Paths::resources_upper_base_obj.string()));
  AnimationEng.addShader(
      RobotParts::upper_base,
      Shader(Paths::shaders_vs.c_str(), Paths::shaders_fs.c_str()), {0, 1, 0});
  container.emplace(RobotParts::middle_arm,
                    Part(Paths::resources_middle_arm_obj.string()));
  AnimationEng.addShader(
      RobotParts::middle_arm,
      Shader(Paths::shaders_vs.c_str(), Paths::shaders_fs.c_str()), {0, 0, 1});
}
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    render(Parts, AnimationEng.getShaderMap(), window);
