...
void PartManager::rotateToPoint(glm::vec3 const &endPos) {
  ...
  float       TargetBaseAngle   = (atan2(y, x));
  ...
  float       TargetMiddleAngle = (atan2(2 * omega * L2 * (bigDelta * bigDelta + omega * omega + L2 * L2 - L3 * L3) + 2 * bigDelta * L2 * sqrt(delta),
                                   2 * bigDelta * L2 * (bigDelta * bigDelta + omega * omega + L2 * L2 - L3 * L3) - 2 * omega * L2 * sqrt(delta)));
  float       TargetJointAngle  = (-TargetMiddleAngle + atan2(omega - L2 * sin(TargetMiddleAngle), bigDelta - L2 * cos(TargetMiddleAngle)));

  if (std::isnan(TargetBaseAngle) || std::isnan(TargetMiddleAngle) || std::isnan(TargetJointAngle) || delta < 0) {
    TargetBaseAngle   = 0;
    TargetJointAngle  = 0;
    TargetMiddleAngle = 0;
  }
